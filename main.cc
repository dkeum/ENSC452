...
void generate_shape_from_rng(int bin_num, int counter,int background_color, double rotation,int elongate_mode, int* array);
...
//	Xil_DCacheDisable();
	...
	while(1){
		...
		if(counter==0){
			generate_param_for_row(bin_num1[6],0);
		}
		if(counter1==-10){
			generate_param_for_row(bin_num2[6],1);
		}
		if(counter2==-10){
			generate_param_for_row(bin_num3[6],2);
		}
		if(counter3==-10){
			generate_param_for_row(bin_num4[6],3);
		}
		if(counter4==-10){
			generate_param_for_row(bin_num5[6],4);
		}
		
		
		

		// keep those values untils the collision for that row has occurred. 
