...
void generate_PRNG(XAxiDma myDma,unsigned long long int time_elapsed){
	...
	status = XAxiDma_SimpleTransfer(&myDma, (u32)output_location, size*sizeof(u32),XAXIDMA_DEVICE_TO_DMA);
	status = XAxiDma_SimpleTransfer(&myDma, (u32)seed, size*sizeof(u32),XAXIDMA_DMA_TO_DEVICE);//typecasting in C/C++
	if(status != XST_SUCCESS){
...
//		printf("the rand num generated: %0x \r\n",output_location[ii]);
