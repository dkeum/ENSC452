...
volatile int program_is_started =0;
...
volatile int one_shape_counter=0;
...
//	one_shape_mode= 0;
    ...
    while(program_is_started==0){}

	XTime_GetTime(&tEnd);

	unsigned long long int temp_var2 = (2*(tEnd-tStart))%300;
	printf("random number seed is: %llu \r\n", temp_var2);

    generate_PRNG(myDma, temp_var2);
...
//	for buttons ---------------------------------------------------
	if(btn_value ==1 ){//center
		...
		if(audio_volume_change >=2){
			...
			audio_lowerVolume();
			is_audio_changed = 2;
			is_light_pattern_changed =1; // only when the audio is lowered the background is reset to deal with remaining pixels from decreasing the size.
			}
		}
