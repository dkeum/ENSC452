...
void generate_PRNG(XAxiDma myDma){
	...
	Xil_DCacheFlushRange((u32)seed,20*3*5*sizeof(u32));
	...
	for(int ii =0; ii < 20;ii++){
		...
		prng_output_location[3*ii] =  abs(a);
		prng_output_location[3*ii +1] =  abs(b); // later on this could be the output from the FFT ip
		prng_output_location[3*ii +2] =  abs(c);
